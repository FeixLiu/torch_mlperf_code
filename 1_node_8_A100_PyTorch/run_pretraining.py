...
grad_scaler = GradScaler(
    init_scale=float(os.getenv("INIT_LOSS_SCALE", 2**20)), growth_interval=2000)
...
class pretraining_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        [
            ...
            if indice < 4 else np.asarray(input[index].astype(np.int64))
        ...
        input_mask[:input_mask_len] = np.ones(
            (1, input_mask_len)).astype(np.int64)
        ...
        return [
            torch.from_numpy(input_ids), torch.from_numpy(segment_ids),
            torch.from_numpy(input_mask), torch.from_numpy(masked_lm_labels),
...
def parse_arguments():
    ...
    parser.add_argument(
        "--use_ddp", default=False, action='store_true', help="Enable DDP.")
    parser.add_argument(
        "--ddp_type",
        default='apex',
        type=str,
...
def found_resume_checkpoint(args):
    ...
    return args.resume_from_checkpoint and len(
...
def prepare_model_and_optimizer(args, device, stream):
    ...
    if args.init_checkpoint is not None or found_resume_checkpoint(args):
        ...
        if args.init_checkpoint is None:  # finding checkpoint in output_dir
            ...
            model_names = [
                f
                for f in glob.glob(
                    os.path.join(args.output_dir, checkpoint_str))
            ]
            global_step = max([
            ...
            resume_init_checkpoint = os.path.join(
                args.output_dir, checkpoint_str.replace("*", str(global_step)))
            print("Setting init checkpoint to %s - which is the latest in %s" %
                  (resume_init_checkpoint, args.output_dir))
            checkpoint = torch.load(resume_init_checkpoint, map_location="cpu")
        else:
            checkpoint = torch.load(
                args.init_checkpoint, map_location="cpu")["model"]

        # not need for v1.0
        # #Log weight initializations
        for weight in utils.convert_weight_names(list(checkpoint.keys())):
            if weight != "cls/predictions/decoder/kernel":
    ...
    mlperf_logger.log_event(
        key=mlperf_logger.constants.OPT_BASE_LR,
    ...
    if args.distributed_lamb:
        ...
        optimizer = DistributedFusedLAMB(
            ...
            fuse_scale=True,
            full_ar=full_ar,
            set_param_views_to_flat_buffer=True,
...
#            optimizer._init_everything()
    ...
    with torch.cuda.stream(stream):
        batch_gpu_placeholder = preprocess_batch(
            args,
            torch.ones(
...
def run_graphed_eval(args,
    ...
    if torch.distributed.is_initialized():
        ...
        torch.distributed.all_reduce(
            mlm_accuracy_eval, op=torch.distributed.ReduceOp.SUM)
        torch.distributed.all_reduce(
            loss_eval, op=torch.distributed.ReduceOp.SUM)
        torch.distributed.all_reduce(
            num_valid_eval, op=torch.distributed.ReduceOp.SUM)
...
def run_eval(args,
    ...
    if torch.distributed.is_initialized():
        ...
        torch.distributed.all_reduce(
            total_masked, op=torch.distributed.ReduceOp.SUM)
...
def exchange_padding_fast(device, input_ids, segment_ids, input_mask,
    ...
    def encode_packet(input_ids, segment_ids, input_mask, masked_lm_labels,
        ...
        packet[curr_pos:curr_pos + flattened_length_seq] = segment_ids.view(
            -1)[:]
        curr_pos += flattened_length_seq

        packet[curr_pos:curr_pos + flattened_length_seq] = input_mask.view(
            -1)[:]
        curr_pos += flattened_length_seq
    ...
    seqlens_, indices = torch.sort(
        input_mask_.sum(dim=2).view(-1), descending=True)
...
def main():
    ...
    if nvtx_flag:
        sbridge.nvtx_push(SBridge.INIT_TIME)
    ...
    mlperf_logger.log_event(
        ...
        sync=False)
    mlperf_logger.log_event(
        key='d_batch_size', value=args.train_batch_size, sync=False)
    mlperf_logger.log_event(
        key=mlperf_logger.constants.GRADIENT_ACCUMULATION_STEPS,
        value=args.gradient_accumulation_steps,
        sync=False)
    mlperf_logger.log_event(
        key='max_predictions_per_seq',
        value=args.max_predictions_per_seq,
        sync=False)
    mlperf_logger.log_event(
        key='opt_learning_rate_training_steps',
    ...
    if 0:  #args.distributed_lamb:
        ...
        loss, _, _ = model(
            input_ids=dummy_data[0],
            token_type_ids=dummy_data[1],
            attention_mask=dummy_data[2],
            masked_lm_labels=dummy_data[4],
            next_sentence_label=dummy_data[5])
        grad_scaler.scale(loss).backward()
        optimizer.complete_reductions()
    ...
    if args.do_train:
        ...
        if found_resume_checkpoint(args):
            ...
            files = [
                ...
                if os.path.isfile(os.path.join(args.input_dir, f)) and 'part' in
                f
            ]
            files.sort()
            num_files = len(files)
        ...
        assert (
            args.gradient_accumulation_steps == 1 and args.fp16
        ) or not args.use_cuda_graph, "code path not tested with cudagraphs"
        assert (
            args.local_rank != -1 and not args.allreduce_post_accumulation
        ) or not args.use_cuda_graph, "code path not tested with cudagraphs"
        ...
        if use_cuda_graph and args.cuda_graph_mode == 'full_iteration':
            batch_gpu_placeholder_multi = [
                preprocess_batch(
                    ...
                    torch.ones(
                        ...
                        device=device),
                    torch.ones(
                        args.train_batch_size, dtype=torch.int64,
                        device=device), )
            ...
            batch_gpu_placeholder_eval = preprocess_batch(
                ...
                torch.ones(
                    ...
                    device=device),
                torch.ones(
                    eval_batch, dtype=torch.int64, device=device), )
            ...
            model = fwd_loss_bwd_trainer.capture_bert_model_segment_graph(
                model, use_cuda_graph)
            current_device = torch.device('cuda', torch.cuda.current_device())
            skipped_steps = torch.zeros(
                (1, ), dtype=torch.int, device=current_device)
        ...
        if use_DDP and not args.distributed_lamb:
            ...
            if args.cuda_graph_mode == 'segmented' or not use_cuda_graph:
                loss, mlm_acc, _ = fwd_loss_bwd_trainer.step(
                    -1, batch_gpu_placeholder, model, optimizer)
                optimizer.zero_grad()
                overflow_buf = None
                ...
                loss_train = torch.zeros(
                    (1, ), dtype=torch.float, device=current_device)
                with torch.cuda.stream(capture_stream):
                    ...
                    if use_cuda_graph:
                        ...
                        for i in range(args.max_iterations_per_graph):
                            optimizer.zero_grad(set_to_none=True)
                            loss, mlm_acc, _ = model(
                                *batch_gpu_placeholder_multi[i])
                            optimizer._lazy_init_stage1()
                            grad_scaler.scale(loss).backward()
                            optimizer._lazy_init_stage2()
                            #static_scale = grad_scaler._scale
                            lr_scheduler.step()
                            optimizer.set_global_scale(
                                grad_scaler._get_scale_async())
                            optimizer.complete_reductions()
                            grad_scaler.step(optimizer)
                            grad_scaler.update()
                            #static_scale.copy_(grad_scaler._scale)
                    ...
                    mlm_accuracy_eval = torch.zeros(
                        (1, ), dtype=torch.float, device=current_device)
                    loss_eval = torch.zeros(
                        (1, ), dtype=torch.float, device=current_device)
                    num_valid_eval = torch.zeros(
                        (1, ), dtype=torch.int64, device=current_device)
                    if use_cuda_graph:
                        model.eval()
                        graph_eval.capture_begin()
                        loss, mlm_acc, num_valid = model(
        ...
        if time_tags:
            torch.cuda.synchronize()
            sbridge.print_tag(SBridge.INIT_TIME,
                              time.time() - init_start,
                              torch.distributed.get_rank())
        if nvtx_flag:
            sbridge.nvtx_pop()  #init_time
        ...
        mlperf_logger.log_start(
            key=mlperf_logger.constants.RUN_START, sync=True)
        ...
        while global_step < args.max_steps and not end_training:
            ...
            if nvtx_flag:
                sbridge.nvtx_push(SBridge.LOAD_TIME)
            ...
            train_data = pretraining_dataset(
                data_file, args.max_predictions_per_seq, args.max_seq_length)
            train_sampler = RandomSampler(train_data)
            train_dataloader = DataLoader(
                train_data,
                sampler=train_sampler,
            ...
            lr_cpu = torch.tensor(
                [0.0], dtype=torch.float32, device='cpu').pin_memory()
            if time_tags:
                torch.cuda.synchronize()
                sbridge.add_tag(SBridge.LOAD_TIME,
                                time.time() - loading_start,
                                torch.distributed.get_rank())
            if nvtx_flag:
                sbridge.nvtx_pop()  #loading_time
            ...
            for f_id in range(f_start_id + 1, len(files)):
                if torch.distributed.get_world_size() > num_files:
                    ...
                    data_file = files[(f_id * torch.distributed.get_world_size()
                                       + torch.distributed.get_rank()) %
                                      num_files]
                ...
                for step, batch in enumerate(train_dataloader):
                    ...
                    if args.exchange_padding == True:
                        ...
                        if forming_multi_iter_batch:
                            for t, t_gpu in zip(
                                    batch, batch_gpu_placeholder_multi[
                                        samples_in_multi_iter_batch]):
                                t_gpu.copy_(t, non_blocking=True)
                            ...
                            if args.max_iterations_per_graph > 1 and next_eval_step - global_step >= args.max_iterations_per_graph and args.max_steps - global_step >= args.max_iterations_per_graph:
                                ...
                                for t, t_gpu in zip(
                                        batch, batch_gpu_placeholder_multi[0]):
                                    t_gpu.copy_(t, non_blocking=True)
                                samples_in_multi_iter_batch = 1
                                continue
                            else:
                                for t, t_gpu in zip(batch,
                    ...
                    if args.cuda_graph_mode == 'segmented' or not use_cuda_graph:
                        loss, mlm_acc, sbridge = fwd_loss_bwd_trainer.step(
                            step, batch, model, optimizer, sbridge, time_tags,
                            nvtx_flag)
                    ...
                    if update_step:
                        ...
                        if time_tags:
                            ...
                            opt_start = time.time()
                        if nvtx_flag:
                            sbridge.nvtx_push(SBridge.OPT_TIME)
                        ...
                        if send_lr_in_parallel:
                            ...
                            with torch.cuda.stream(stats_stream):
                                lr_cpu.copy_(
                                    optimizer._lr.detach(), non_blocking=True)
                        else:
                            now_lr = optimizer.param_groups[0]['lr']

                        if args.cuda_graph_mode == 'segmented' or not use_cuda_graph:
                            global_step = take_optimizer_step(
                                args, optimizer, model, overflow_buf,
                                global_step)
                        ...
                        if time_tags:
                            ...
                            if not sbridge.add_tag(
                                ...
                                time_tags = 0
                        if nvtx_flag:
                            sbridge.nvtx_pop()  #opt_time

                        if (args.eval_dir and args.eval_iter_samples > 0 and
                                global_step == next_eval_step):
                            if time_tags:
                                torch.cuda.synchronize()
                                eval_start = time.time()
                            if nvtx_flag:
                                sbridge.nvtx_push(SBridge.EVAL_TIME)
                            ...
                            if skip_interval > 0:
                                ...
                                if eval_count == 0:
                                    eval_dataloader = eval_dataset_future.result(
                                        timeout=None)
                                ...
                                if args.cuda_graph_mode == 'segmented' or not use_cuda_graph:
                                    ...
                                    eval_avg_loss, eval_avg_mlm_accuracy = run_graphed_eval(
                                        args,
                                        graph_eval,
                                        batch_gpu_placeholder_eval,
                                        eval_dataloader,
                            ...
                            if time_tags:
                                torch.cuda.synchronize()
                                sbridge.add_tag(SBridge.EVAL_TIME,
                                                time.time() - eval_start,
                                                torch.distributed.get_rank())
                            if nvtx_flag:
                                sbridge.nvtx_pop()  #eval_time
                    ...
                    if args.log_freq > 0 and training_steps % (
                        ...
                        if utils.is_main_process():
                            ...
                            mlperf_logger.log_event(
                                key='throughput', value=training_perf)
                    ...
                    if global_step >= args.max_steps or end_training:
                        ...
                        if args.log_freq > 0:
                            last_num_steps = int(
                                training_steps / args.
                                gradient_accumulation_steps) % args.log_freq
                    ...
                    if end_training or (samples_trained - samples_trained_prev
                                        >= args.num_samples_per_checkpoint and
                                        samples_trained >=
                                        args.min_samples_to_start_checkpoints):
                        samples_trained_prev = samples_trained
            ...
            if time_tags or nvtx_flag:
                ...
                mlperf_logger.log_event(
                    key=SBridge.EPOCH_TIME,
                    value={'r': torch.distributed.get_rank(),
                           't': epoch_time},
                    log_all_ranks=True,
                    sync=False)
        if time_tags:
        ...
        mlperf_logger.log_event(
            key=mlperf_logger.constants.TRAIN_SAMPLES,
