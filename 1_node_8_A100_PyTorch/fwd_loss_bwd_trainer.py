...
def preprocess_batch(args,
    ...
    if args.pad_fmha:
        ...
        cu_seqlens = torch.zeros(
            b + 1, dtype=torch.int32, device=seqlens.device)
        cu_seqlens[1:] = torch.cumsum(seqlens, 0)

        position_ids = torch.cat([
            torch.arange(
                s, dtype=torch.int64, device=seqlens.device) for _ in range(b)
        ]).view(b, s)
...
class FwdLossBwdTrainer():
    def __init__(self, args, grad_scaler):
        ...
        self.loss_cpu = torch.tensor(
            0.0, dtype=torch.float32, device='cpu').pin_memory()
        self.mlm_acc_cpu = torch.tensor(
            0.0, dtype=torch.float32, device='cpu').pin_memory()
    ...
    def capture_bert_model_segment_graph(self, bert_model, use_cuda_graph):
        ...
        sample_eval = [
            ...
            torch.ones(
                ...
                device=self.args.device),
            torch.ones(
                eval_batch, dtype=torch.int64, device=self.args.device),
        ]
        sample_model_train = preprocess_batch(
            self.args, *sample_train, input_only=True)
        sample_model_eval = preprocess_batch(
            self.args, *sample_eval, input_only=True)
        bert_model_segment = graph(
            bert_model_segment,
            tuple(t.clone() for t in sample_model_train),
            tuple(t.clone() for t in sample_model_eval)
            if self.args.eval_batch_size * world_size >=
            self.args.num_eval_examples else None,
            self.capture_stream,
            warmup_iters=8,
        ...
        sample_head_tuple_train = tuple([
            sample_head_train[0].clone().requires_grad_(),
            sample_head_train[1].clone().requires_grad_(),
            sample_head_train[2].clone(), sample_head_train[3].clone()
        ])
        sample_head_tuple_eval = tuple([
            sample_head_eval[0].clone(), sample_head_eval[1].clone(),
            sample_head_eval[2].clone(), sample_head_eval[3].clone()
        ])
        bert_head_segment = graph(
            bert_head_segment,
            sample_head_tuple_train,
            sample_head_tuple_eval if self.args.eval_batch_size * world_size >=
            self.args.num_eval_examples else None,
            self.capture_stream,
            warmup_iters=8,
            warmup_only=(not use_cuda_graph))
    ...
    def step(self,
        ...
        fwd_start = bwd_start = 0
        if time_tags:
            torch.cuda.synchronize()
            fwd_start = time.time()
        if nvtx_flag:
            sbridge.nvtx_push(SBridge.FWD_TIME)
        ...
        if time_tags:
            torch.cuda.synchronize()
            bwd_start = time.time()
            sbridge.add_tag(SBridge.FWD_TIME, bwd_start - fwd_start,
                            torch.distributed.get_rank())
        if nvtx_flag:
            sbridge.nvtx_pop()  #fwd_time
            sbridge.nvtx_push(SBridge.BWD_TIME)
        ...
        if self.args.bypass_amp:
            ...
            with amp.scale_loss(
                    loss,
                    optimizer,
                    delay_overflow_check=self.args.
                    allreduce_post_accumulation) as scaled_loss:
                scaled_loss.backward()
        if time_tags:
            torch.cuda.synchronize()
            sbridge.add_tag(SBridge.BWD_TIME,
                            time.time() - bwd_start,
                            torch.distributed.get_rank())
        if nvtx_flag:
            sbridge.nvtx_pop()  #bwd_time
